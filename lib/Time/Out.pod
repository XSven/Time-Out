...
C<Time::Out> provides an easy interface to alarm(2) based timeouts.  Nested
timeouts are supported.


=head2 RETURN VALUE

C<timeout()> returns whatever the code placed inside the block returns:
...
eval, closures, alarm(2), L<Sys::AlarmCall>
